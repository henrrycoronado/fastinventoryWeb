...
export const purchasesApi = {
  companies: {
    create: (data: { name: string }) =>
      apiClient.post(`/api/purchases/companies`, data).then(r => r.data),
    update: (companyCen: string, data: { name: string; isActive: boolean }) =>
      apiClient.put(`/api/purchases/companies/${companyCen}`, data).then(r => r.data),
    createWarehouse: (companyCen: string, data: { name: string }) =>
      apiClient.post(`/api/purchases/companies/${companyCen}/warehouses`, data).then(r => r.data),
    updateWarehouse: (companyCen: string, warehouseCen: string, data: { name: string; isActive: boolean }) =>
      apiClient.put(`/api/purchases/companies/${companyCen}/warehouses/${warehouseCen}`, data).then(r => r.data),
  },
