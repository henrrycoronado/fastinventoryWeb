...
export default function CompanyProfile() {
  ...
  const [deleteTarget, setDeleteTarget]         = useState<{ cen: string; name: string } | null>(null)
  ...
  const deleteWarehouse = useMutation({
    ...
    mutationFn: () => Promise.resolve(), // warehouseApi.delete(companyCen!, deleteTarget!.cen),
    onSuccess:  () => {
      qc.invalidateQueries({ queryKey: ['warehouses', companyCen] })
      if (selectedWarehouse?.cen === deleteTarget?.cen) setWarehouse(null)
        ...
        <div>
          ...
          <div className="card divide-y divide-surface-4">
            {(warehouses as WarehouseType[]).length === 0 ? (
              ...
              <div key={w.cen} className="flex items-center gap-4 px-5 py-4">
                ...
                <button
                  onClick={() => setDeleteTarget({ cen: w.cen, name: w.name })}
