...
// ── SKU selector ─────────────────────────────────────────────────────────
function ProductSkuSelector({ onSelect }: {
  onSelect: (sku: Sku & { productName: string }) => void
}) {
      ...
      <div className="divide-y divide-surface-3">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2">
            ...
            <p className="text-sm text-ink-muted">Sin productos encontrados</p>
          </div>
            ...
            <div key={p.id}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-ink-primary truncate">{nombre}</p>
                  <p className="text-xs text-ink-muted">
                    {p.globalProduct?.brand ?? '—'} · {p.skus?.length ?? 0} SKU(s)
                  </p>
                </div>
                <ChevronRight
                  size={13}
              ...
              {isExpanded && (
                <div className="bg-surface-1 px-4 pb-3 space-y-1 animate-slide-up">
                  {!p.skus?.length ? (
                    <p className="text-xs text-ink-muted px-3 py-2">Sin SKUs registrados</p>
                  ) : p.skus.map((sku: Sku) => (
                    <button
                      key={sku.id}
                      onClick={() => onSelect({ ...sku, productName: nombre })}
...
// ── Kardex table ──────────────────────────────────────────────────────────
function KardexTable({ skuId, productName, skuLabel }: {
  skuId:       number
  productName: string
  skuLabel:    string
}) {
  ...
  const stats = useMemo(() => {
    ...
    const totalEntradas = entries
      .filter(e => [1, 3, 6].includes(e.typeId))
      .reduce((a, e) => a + e.quantity, 0)
    const totalSalidas = entries
      .filter(e => [2, 4, 5].includes(e.typeId))
      .reduce((a, e) => a + e.quantity, 0)
    const balanceFinal = entries.length > 0
  ...
  if (isLoading) return (
    <div className="flex justify-center py-16 text-sm text-ink-muted">
      Cargando kardex…
    </div>
  )
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="card p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-ink-primary">{productName}</p>
            <p className="text-xs font-mono text-ink-muted">{skuLabel} · {selectedWarehouse?.name}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card p-4 text-center">
          <p className="text-xs text-ink-muted uppercase tracking-wider">Total entradas</p>
          <p className="font-display text-xl font-bold text-accent mt-1">{stats.totalEntradas}</p>
          <p className="text-xs text-ink-muted">unidades</p>
        </div>
        <div className="card p-4 text-center">
          <p className="text-xs text-ink-muted uppercase tracking-wider">Total salidas</p>
          <p className="font-display text-xl font-bold text-red-400 mt-1">{stats.totalSalidas}</p>
          <p className="text-xs text-ink-muted">unidades</p>
        </div>
        <div className="card p-4 text-center">
          <p className="text-xs text-ink-muted uppercase tracking-wider">Balance actual</p>
          <p className="font-display text-xl font-bold text-ink-primary mt-1">{stats.balanceFinal}</p>
          <p className="text-xs text-ink-muted">unidades</p>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="grid grid-cols-5 px-6 py-3 border-b border-surface-4">
          {['Fecha', 'Tipo', 'Entrada', 'Salida', 'Balance'].map(h => (
        ...
        {entries.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-2">
            <ScrollText size={24} className="text-ink-muted" />
              ...
              return (
                <div key={e.id} className="grid grid-cols-5 items-center px-6 py-3 hover:bg-surface-2/50 transition-colors">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-ink-secondary truncate">{e.typeName}</span>
                  </div>
                  <span className={`text-xs font-mono font-medium ${isEntry ? 'text-accent' : 'text-ink-muted'}`}>
                    {isEntry ? `+${e.quantity}` : '—'}
                  </span>
                  <span className={`text-xs font-mono font-medium ${!isEntry ? 'text-red-400' : 'text-ink-muted'}`}>
                    {!isEntry ? `-${e.quantity}` : '—'}
                  </span>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-surface-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-ink-primary">Kardex</h1>
        ...
        {!selectedSku ? (
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b border-surface-4">
              <p className="text-xs text-ink-muted">
                Selecciona un producto y luego un SKU para ver su historial de movimientos
              </p>
            </div>
            <div className="p-4">
