...
export default function InventoryDashboard() {
  ...
  const stockByCategory = useMemo(() => {
    ...
    stockList.forEach((s: Stock) => {
      const catName = s.sku?.companyProduct?.globalProduct?.category?.name ?? 'Sin categoría'
              ...
              {recentMovements.map((m: Movement) => {
                ...
                return (
                  <div key={m.id} className="flex items-center justify-between px-5 py-3">
                    <div>
                      <p className="text-xs font-medium text-ink-primary">{info?.label ?? `Tipo ${m.typeId}`}</p>
                      <p className="text-xs text-ink-muted">
                        {new Date(m.movementDate).toLocaleDateString('es-BO', {
                          day: '2-digit', month: 'short', year: 'numeric',
                        })}
                      </p>
                    </div>
                    <span className={`badge ${info?.isEntry ? 'bg-accent/10 text-accent' : 'bg-red-500/10 text-red-400'}`}>
                      {info?.isEntry ? 'Entrada' : 'Salida'}
                    </span>
                  </div>
                )
              })}
                  ...
                  <div key={s.id} className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-ink-muted font-mono">{sku}</p>
                      </div>
                    </div>
                    <span className={`badge shrink-0 ml-2 ${isCritical ? 'bg-red-500/10 text-red-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
